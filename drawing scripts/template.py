...
def draw_arrow_with_label(
    ...
    """
    Draw an arrow connector with a text label positioned along the line.
    ...
    Args:
        ...
        label_pos: Position of label along the line (0-1, default: 0.5 = middle)
        color: Arrow color (default: BLUE)
        linewidth: Line width
        arrow_size: Size of the arrow head
        text_color: Text color (default: BLACK)
        text_size: Font size (default: DEFAULT_SIZE * 0.7)

    Returns:
        Tuple of (annotation object, text object)
    """
    if text_size is None:
        text_size = DEFAULT_SIZE * 0.7
    ...
    label_x = p1[0] + (p2[0] - p1[0]) * label_pos
    label_y = p1[1] + (p2[1] - p1[1]) * label_pos
...
def draw_logic_gate(ax, gate_type, x, y, size=1.5, inputs=2, label="", text_size=None):
    """Draw logic gate symbol (AND, OR, NOT, NAND, NOR, XOR)."""
    if text_size is None:
        text_size = int(DEFAULT_SIZE * 0.7)
    ...
    if gate_type == "NOT":
        ...
        ax.add_patch(triangle)
        draw_circle(ax, x + size / 2 + 0.15, y, 0.1, "", border_color=BLUE)
    elif gate_type == "AND":
        draw_box(
            ax,
            x - size / 2,
        ...
        poly = patches.Polygon(pts, facecolor=WHITE, edgecolor=BLUE, linewidth=2)
        ax.add_patch(poly)
    else:
        # Generic gate box
        draw_box(
    ...
    for i in range(inputs):
        inp_y = y + (i - (inputs - 1) / 2) * size / 3
        ax.plot(
            [x - size / 2 - 0.5, x - size / 2],
            [inp_y, inp_y],
            color=BLACK,
            linewidth=2,
            zorder=10,
    ...
    # Output line
    ax.plot(
        [x + size / 2, x + size / 2 + 0.5], [y, y], color=BLACK, linewidth=2, zorder=10
    )
...
def draw_alu_block(ax, x, y, operations, width=3, height=2, text_size=None):
    """Draw ALU component with operation list."""
    if text_size is None:
        text_size = int(DEFAULT_SIZE * 0.8)
    draw_box(
        ax,
...
def draw_memory_array(
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            if addresses and i * cols + j < len(addresses):
                ax.text(
                    cell_x + cell_w / 2,
                    cell_y - cell_h / 2,
                    hex(addresses[i * cols + j]),
                    fontsize=DEFAULT_SIZE * 0.5,
                    ha="center",
                    zorder=20,
                )
...
def draw_logic_gate(
    ...
    """
    Draw a logic gate symbol using polygon shapes.

    Args:
        ax: matplotlib axes
        gate_type: Type of gate - "AND", "OR", "NOT", "NAND", "NOR", "XOR"
        x, y: Center position
        size: Size of the gate (default: 1.5)
        inputs: Number of inputs (default: 2)
        label: Optional label text
        text_size: Font size (default: DEFAULT_SIZE)

    Returns:
        The main Polygon patch object
    """
    if text_size is None:
        text_size = DEFAULT_SIZE
...
def draw_alu_block(ax, x, y, operations, width=3, height=2, text_size=None):
    """
    Draw an ALU (Arithmetic Logic Unit) component with operation list.

    Args:
        ax: matplotlib axes
        x, y: Bottom-left corner position
        operations: List of operation names (e.g., ["ADD", "SUB", "AND", "OR"])
        width: Box width (default: 3)
        height: Box height (default: 2)
        text_size: Font size (default: DEFAULT_SIZE)
...
def draw_logic_gate(ax, gate_type, x, y, size=1.5, inputs=2, label="", text_size=None):
    ...
    if gate_type == "AND":
        ...
        t = np.linspace(0, np.pi, 20)
        left_curve = [
            (x - w / 2 + 0.3 + 0.3 * np.cos(ang), y + 0.5 * h * np.sin(ang))
            for ang in t
        ]
        right_curve = [
            (x + w / 2 - 0.3 * (1 - np.sin(ang)), y + 0.5 * h * np.sin(ang))
            for ang in t[::-1]
        ]
        vertices = (
            [(x - w / 2, y - h / 2), (x - w / 2, y + h / 2)] + left_curve + right_curve
        ...
        vertices = [(x - w / 2, y - h / 2), (x - w / 2, y + h / 2), (x + w / 3, y)]
        gate = patches.Polygon(
            vertices,
            closed=True,
            facecolor=fill_color,
...
def draw_memory_array(
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            ax.add_patch(cell)
            cell_positions[f"{row}_{col}"] = (cell_x + cell_w / 2, cell_y + cell_h / 2)
...
def draw_page_table(
    ...
    """
    Draw MMU page table visualization.

    Args:
        ax: matplotlib axes
        entries: List of tuples (vpn, ppn, valid, flags)
                 - vpn: Virtual Page Number
                 - ppn: Physical Page Number (or "-" if invalid)
                 - valid: Boolean (valid/invalid)
                 - flags: String (e.g., "RWX", "RO", "R--")
        x, y: Top-left corner position
        size: Font size (default: DEFAULT_SIZE * 0.7)
        cell_w: Cell width
        cell_h: Cell height
    ...
    # Draw entries
    for i, (vpn, ppn, valid, flags) in enumerate(entries):
        row_y = y - (i + 2) * cell_h
        ...
        # PPN cell
        fill = LIGHT_GREEN if valid else LIGHT_RED
        border = GREEN if valid else RED
        draw_box(
            ax,
            x + cell_w,
            row_y,
        ...
        valid_color = GREEN if valid else RED
        draw_box(
            ax,
            x + 2 * cell_w,
            row_y,
    ...
    return {"width": n_cols * cell_w, "height": n_rows * cell_h}
...
def draw_logic_gate(ax, gate_type, x, y, size=1.5, inputs=2, label="", text_size=None):
    ...
    if gate_type == "AND":
        vertices = [(x - w/2, y - h/2), (x - w/2, y + h/2), (x + w/6, y + h/2), (x + w/2, y + h/4), (x + w/2, y - h/4), (x + w/6, y - h/2)]
        gate = patches.Polygon(vertices, closed=True, facecolor=fill_color, edgecolor=border_color, linewidth=2, zorder=5)
        ax.add_patch(gate)
    elif gate_type == "OR":
        t = np.linspace(0, np.pi, 20)
        left_curve = [(x - w/2 + 0.3 + 0.3*np.cos(ang), y + 0.5*h*np.sin(ang)) for ang in t]
        right_curve = [(x + w/2 - 0.3*(1-np.sin(ang)), y + 0.5*h*np.sin(ang)) for ang in t[::-1]]
        vertices = [(x - w/2, y - h/2), (x - w/2, y + h/2)] + left_curve + right_curve
        gate = patches.Polygon(vertices, closed=True, facecolor=fill_color, edgecolor=border_color, linewidth=2, zorder=5)
        ax.add_patch(gate)
    elif gate_type == "NOT":
        vertices = [(x - w/2, y - h/2), (x - w/2, y + h/2), (x + w/3, y)]
        gate = patches.Polygon(vertices, closed=True, facecolor=fill_color, edgecolor=border_color, linewidth=2, zorder=5)
        ax.add_patch(gate)
        bubble = patches.Circle((x + w/3 + 0.15, y), 0.12, facecolor=fill_color, edgecolor=border_color, linewidth=2, zorder=5)
        ax.add_patch(bubble)
    elif gate_type == "NAND":
        vertices = [(x - w/2 - 0.2, y - h/2), (x - w/2 - 0.2, y + h/2), (x + w/6 - 0.2, y + h/2), (x + w/2 - 0.2, y + h/4), (x + w/2 - 0.2, y - h/4), (x + w/6 - 0.2, y - h/2)]
        gate = patches.Polygon(vertices, closed=True, facecolor=fill_color, edgecolor=border_color, linewidth=2, zorder=5)
        ax.add_patch(gate)
        bubble = patches.Circle((x + w/2 - 0.05, y), 0.12, facecolor=fill_color, edgecolor=border_color, linewidth=2, zorder=5)
        ax.add_patch(bubble)
    elif gate_type == "NOR":
        vertices = [(x - w/2 - 0.1, y - h/2), (x - w/2 - 0.1, y + h/2), (x - w/6, y + h/2), (x + w/4, y + h/4), (x + w/4, y - h/4), (x - w/6, y - h/2)]
        gate = patches.Polygon(vertices, closed=True, facecolor=fill_color, edgecolor=border_color, linewidth=2, zorder=5)
        ax.add_patch(gate)
        bubble = patches.Circle((x + w/4 + 0.15, y), 0.12, facecolor=fill_color, edgecolor=border_color, linewidth=2, zorder=5)
        ax.add_patch(bubble)
    elif gate_type == "XOR":
        vertices = [(x - w/2 - 0.2, y - h/2), (x - w/2 - 0.2, y + h/2), (x - w/6, y + h/2), (x + w/4, y + h/4), (x + w/4, y - h/4), (x - w/6, y - h/2)]
        gate = patches.Polygon(vertices, closed=True, facecolor=fill_color, edgecolor=border_color, linewidth=2, zorder=5)
        ax.add_patch(gate)
        xor_curve = patches.Arc((x - w/2 - 0.1, y), h*0.8, h*0.8, angle=0, theta1=-60, theta2=60, color=border_color, linewidth=2, zorder=5)
        ax.add_patch(xor_curve)
    else:
        gate = patches.Rectangle((x - w/2, y - h/2), w, h, facecolor=fill_color, edgecolor=border_color, linewidth=2, zorder=5)
        ax.add_patch(gate)

    input_spacing = h / (inputs + 1)
    for i in range(inputs):
        input_y = y - h/2 + (i + 1) * input_spacing
        ax.plot([x - w/2 - 0.5, x - w/2 - 0.05], [input_y, input_y], color=border_color, linewidth=2, zorder=5)

    if gate_type in ["NOT", "NAND", "NOR"]:
        ax.plot([x + w/3 + 0.25, x + w/3 + 0.5], [y, y], color=border_color, linewidth=2, zorder=5)
    elif gate_type == "XOR":
        ax.plot([x + w/4 + 0.1, x + w/4 + 0.5], [y, y], color=border_color, linewidth=2, zorder=5)
    else:
        ax.plot([x + w/2 + 0.05, x + w/2 + 0.5], [y, y], color=border_color, linewidth=2, zorder=5)

    ax.text(x, y, gate_type, color=BLACK, fontproperties=get_code_font_prop(int(text_size * 0.8)), ha="center", va="center", zorder=20)

    if label:
        ax.text(x, y - h/2 - 0.4, label, color=GRAY, fontproperties=get_font_prop(int(text_size * 0.7)), ha="center", va="top", zorder=20)
    return gate
...
def draw_alu_block(ax, x, y, operations, width=3, height=2, text_size=None):
    ...
    ax.text(x + width/2, y + height - 0.3, "ALU", color=BLUE, fontproperties=get_font_prop(int(text_size * 1.1)), ha="center", va="center", zorder=20)
    ...
    ax.plot([x + width/2 - 0.5, x + width/2 + 0.5], [y, y - 0.1], color=border_color, linewidth=1.5, zorder=5)
    return box


def draw_memory_array(ax, x, y, rows=4, cols=4, cell_w=0.8, cell_h=0.5, addresses=None, labels=None):
    border_color = BLUE
    fill_color = WHITE
    cell_positions = {}
    ...
    for row in range(rows):
        for col in range(cols):
            cell_x = x + col * cell_w
            cell_y = y - (row + 1) * cell_h
            cell = patches.Rectangle((cell_x, cell_y), cell_w, cell_h, facecolor=fill_color, edgecolor=border_color, linewidth=1.5, zorder=5)
            ax.add_patch(cell)
            cell_positions[f"{row}_{col}"] = (cell_x + cell_w/2, cell_y + cell_h/2)

            if addresses and row * cols + col < len(addresses):
                ax.text(cell_x + cell_w/2, cell_y + cell_h + 0.08, addresses[row * cols + col], color=GRAY, fontproperties=get_code_font_prop(int(DEFAULT_SIZE * 0.5)), ha="center", va="bottom", zorder=20)

            if labels and row * cols + col < len(labels):
                ax.text(cell_x + cell_w/2, cell_y + cell_h/2, labels[row * cols + col], color=BLACK, fontproperties=get_code_font_prop(int(DEFAULT_SIZE * 0.6)), ha="center", va="center", zorder=20)
            else:
                ax.text(cell_x + cell_w/2, cell_y + cell_h/2, "—", color=GRAY, fontproperties=get_code_font_prop(int(DEFAULT_SIZE * 0.5)), ha="center", va="center", zorder=20)

    for col in range(cols):
        col_x = x + col * cell_w + cell_w/2
        ax.text(col_x, y + 0.15, f"C{col}", color=BLUE, fontproperties=get_code_font_prop(int(DEFAULT_SIZE * 0.5)), ha="center", va="bottom", zorder=20)

    for row in range(rows):
        row_y = y - row * cell_h - cell_h/2
        ax.text(x - 0.2, row_y, f"R{row}", color=BLUE, fontproperties=get_code_font_prop(int(DEFAULT_SIZE * 0.5)), ha="center", va="center", zorder=20)

    memory_border = patches.Rectangle((x - 0.1, y - rows * cell_h - 0.1), cols * cell_w + 0.2, rows * cell_h + 0.3, facecolor="none", edgecolor=BLUE, linewidth=2, linestyle="--", zorder=4)
    ax.add_patch(memory_border)
    return cell_positions
