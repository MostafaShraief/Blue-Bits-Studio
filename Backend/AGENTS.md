...
### 6. Integration
Delegates to built-in ASP.NET Core middleware (CORS, compression, auth) and existing project extensions (`AddRateLimiting(configuration)` from `RateLimitingExtensions`, `AddSwaggerWithConfig` from `SwaggerExtensions`). Configures EF Core SQLite via `BlueBitsDbContext`.
