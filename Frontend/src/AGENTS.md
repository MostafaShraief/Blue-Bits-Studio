...
### 6. Integration
Calls backend via `fetchStats()` and `fetchSessions(1, 5)` from `../utils/api`. Uses `useAuth()` context for RBAC checks and user role detection. Stat cards and quick actions are derived from `user.allowedWorkflows` (the frontend mapping of `authorizedWorkflows`).
